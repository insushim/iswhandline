...
- **결혼선**: 새끼손가락 아래 옆면 가로선
- **재물선**: 생명선 안쪽의 작은 세로선들
- **건강선**: 손바닥 중앙에서 수성구로 향하는 사선
- **직감선**: 월구에서 수성구로 향하는 곡선 (희귀)
- **금성대**: 감정선 위에 검지-약지를 연결하는 반원 (희귀)
...
- 화성 평원(손바닥 중앙): 에너지, 투쟁심
- 상화성구(화성 평원 위): 정신적 용기
- 하화성구(화성 평원 아래): 육체적 용기

### 5. ⭐ 특수 손금 (매우 중요 - 반드시 확인)

#### 막손금 (원숭이선, Simian Line) - 인구의 약 1-3%
**정의**: 두뇌선과 감정선이 하나로 합쳐져 손바닥을 가로지르는 단일 선
**특징**:
- 일반인은 두뇌선(아래)과 감정선(위)이 분리되어 있음
- 막손금은 이 두 선이 완전히 합쳐져 하나의 강한 가로선으로 나타남
- 손바닥 한쪽 끝에서 다른 쪽 끝까지 일직선으로 이어짐
**성격 해석**:
- 극도의 집중력과 몰입 능력 (한 가지에 완전히 빠져듦)
- 이성과 감정이 분리되지 않음 (올인하거나 완전히 무관심)
- 흑백논리, 중간이 없음 (좋아하거나 싫어하거나)
- 강한 의지력과 고집
- 독창적이고 비범한 재능
- 역사적 인물, 천재, 예술가에게서 많이 발견
**긍정적 측면**: 뛰어난 집중력, 독특한 재능, 강한 추진력, 전문가적 성취
**주의점**: 완고함, 스트레스 관리 어려움, 인간관계에서 융통성 부족

#### 시드니선 (Sydney Line)
**정의**: 두뇌선이 손바닥 끝까지 매우 길게 뻗은 선 (감정선은 정상)
**특징**: 두뇌선이 새끼손가락 쪽 끝까지 이어짐
**해석**: 과잉 사고, 완벽주의, 세밀한 분석력, 때때로 걱정이 많음

#### M자 손금
**정의**: 생명선+두뇌선+감정선+운명선이 'M' 자 형태를 형성
**해석**: 재물운과 성공운이 매우 좋음, 직관력이 뛰어남

#### 작가의 포크 (Writer's Fork)
**정의**: 두뇌선 끝이 두 갈래로 갈라짐
**해석**: 창의력과 논리력의 조화, 글쓰기 재능

#### 신비의 십자 (Mystic Cross)
**정의**: 두뇌선과 감정선 사이 손바닥 중앙에 있는 X 모양
**해석**: 직관력, 영적 능력, 초자연적 관심

#### 결혼선의 변형
- 긴 결혼선: 깊고 오래가는 관계
- 짧은 결혼선: 짧은 연애 또는 가벼운 관계
- 갈라진 끝: 관계의 갈등 또는 별거 가능성
- 위로 휜 끝: 결혼을 늦추거나 독신 선호
- 아래로 휜 끝: 파트너보다 오래 살 가능성

#### 직감선 (Line of Intuition)
**정의**: 월구에서 시작해 수성구로 향하는 반원형 선
**해석**: 매우 희귀, 강한 직관력과 예지력, 심리학/상담 분야 적성

#### 솔로몬의 반지 (Ring of Solomon)
**정의**: 검지 아래 목성구를 감싸는 반원형 선
**해석**: 지혜, 리더십, 타인을 이해하는 능력

#### 토성의 반지 (Ring of Saturn)
**정의**: 중지 아래 토성구를 감싸는 반원형 선
**해석**: 고독, 우울한 성향, 깊은 사색가

### 6. 특수 기호
- 별 (★): 행운, 돌발적 사건. 위치에 따라 의미 다름
- 십자 (+): 장애물, 변화. 두뇌선-감정선 사이면 신비력
- 삼각형 (△): 성공, 재능. 손바닥 중앙이면 재물운
- 사각형 (□): 보호, 안정. 선 위에 있으면 위험 방지
- 섬 (○): 장애물, 건강 주의. 선 위에 있으면 해당 시기 주의
- 격자 (#): 혼란, 방해. 구 위에 있으면 해당 분야 어려움
- 점: 일시적 장애나 사건
- 사슬 모양: 불안정, 감정기복 (감정선에 많으면 감정 불안정)

### 7. 양손 비교
- 비우세손 (보통 왼손): 타고난 잠재력, 내면의 모습
- 우세손 (보통 오른손): 현재 상태, 노력으로 만든 모습
- 양손이 많이 다르면 → 자기 개발을 많이 한 사람
...
## ⭐ 막손금(원숭이선) 감지 시 특별 지침
만약 두뇌선과 감정선이 하나로 합쳐진 막손금(Simian Line)이 관찰되면:
1. specialMarks.simianLine: true로 설정
2. specialMarks.simianLineDescription에 상세 설명 기재
3. personality.summary에 "막손금(원숭이선)"임을 명시하고 특수성 강조
4. specialNotes에 막손금의 의미를 상세히 설명
5. 성격 분석에서 극도의 집중력, 올인 성향, 흑백논리 등 특성 반영
6. 직업운에서 전문가, 연구자, 예술가 등 몰입이 필요한 분야 강조
7. 인간관계 조언에서 유연성과 타협의 중요성 언급
